...
const (
	// BridgeMemoVersion is the string version of the current memo, used in the memo json we expect.
	BridgeMemoVersion = "coreumbridge-xrpl-v1"
)
...
func DecodeCoreumRecipientFromMemo(memos rippledata.Memos) sdk.AccAddress {
	...
	for _, memo := range memos {
		...
		if err := json.Unmarshal(memo.Memo.MemoData.Bytes(), &bridgeMemo); err != nil {
			return nil
		}
		if bridgeMemo.Version != BridgeMemoVersion {
			return nil
		}
