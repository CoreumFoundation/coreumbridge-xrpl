...
func TestInitAndReadBootstrappingConfig(t *testing.T) {
	...
	require.YAMLEq(t, getDefaultBootstrappingConfigString(), string(yamlStringConfig))
...
func TestInitAndReadKeysRotationConfig(t *testing.T) {
	...
	yamlStringConfig, err := yaml.Marshal(defaultCfg)
	require.NoError(t, err)
	require.YAMLEq(t, getDefaultKeysRotationConfigString(), string(yamlStringConfig))
