...
`token decimals` (always 15), `sending precision`. The `sending precision` should be provided taking into
...
tokens. Similar to XRPL native tokens the XRP token has the `sending precision`.
...
`coreum denom`, `token decimals`, `sending precision` and `fees`. The `sending precision` should be provided
