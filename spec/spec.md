...
##### Usage of the LatestLedger

XRPL transactions should contain the `LastLedgerSequence` to be reliably submitted (more
info [here](https://xrpl.org/reliable-transaction-submission.html)).
The XRP Ledger never includes a transaction in a ledger version whose ledger index is higher than the transaction's
`LastLedgerSequence` parameter. It is important to set the `LastLedgerSequence` since the transaction might be submitted
and stuck in the execution queue for an unpredictable time.

###### LastLedgerSequence computation

The relayers should add the `LastLedgerSequence` which can be calculated using the formula:

```
lastLedgerSequence = currentLedger + lastLedgerSequenceDelay
```

Since the `currentLedger` might be different for all relayers at the time they build the transaction and provide
the signature, we should update the formula to reach the same result taking into account that the
`currentLedger` might be not significantly, but different.

```
lastLedgerSequence = (currentLedger / minLastLedgerSequenceDelay * minLastLedgerSequenceDelay) + 2 * minLastLedgerSequenceDelay
```
...
```
currentLedger = 1050 
minLastLedgerSequenceDelay = 100
lastLedgerSequence = (1050 / 100 * 100) + 2 * 100 = 1200
(1200 - 100) >= 1050 
```
...
currentLedger = 1100 
minLastLedgerSequenceDelay = 100
lastLedgerSequence = (1100 / 100 * 100) + 2 * 100 = 1300
(1300 - 100) >= 1100 
```

The provided formula helps to define the `lastLedgerSequence` in the deterministic way and final value greater than
`currentLedger` in the conditions:

* all relayers XRP nodes have the same `currentLedger`
* all relayers compute the `lastLedgerSequence` withing the time less than the time needed for the XRP node to reach  
  `currentLedger` + `minLastLedgerSequenceDelay`.

In real life the `minLastLedgerSequenceDelay`should include all potential delays and correlations.

###### LastLedgerSequence usage

Once the `LastLedgerSequence` is computed it can be used for the transaction to be signed. In normal scenario the
relayers will come up to the same result, provide their signatures and once the signing threshold is reached submit the
transaction. But there is a chance for to come up to a different `lastLedgerSequence` even having the same
`minLastLedgerSequenceDelay` current ledger. Because each relayers compute the `lastLedgerSequence` for the same
operation at different time, so they might face the state:
...
currentLedger = 1099
minLastLedgerSequenceDelay = 100
lastLedgerSequence = (1099 / 100 * 100) + 2 * 100 = 1200
(1200 - 100) >= 1099
...
The example shows that small difference in the state might lead to the inconsistency. So it should be possible either to
confirm the same `lastLedgerSequence` before the providing signature or let the relayers update the signature. The first
option will require the extra steps (and gas) for the confirmation and there is still a chance to choose the wrong
`lastLedgerSequence` which is less than `currentLedger`, which will lead to the second option anyway. Hence, we need
to allow the relayer update the signature for the operation using the algorithm:

* get operation
* get `currentLedger`
* compute `lastLedgerSequence`
* if the operation has no signatures of the current relayer provide the signature
* else check if the signature is the same
    * if same exit
    * else update the signature

Such approach helps to handle inconsistency of the relayers, since the relayers filter the invalid signature before 
the transaction submission. Also, it helps to handle the case when the majority of the relayers came up to 
the `lastLedgerSequence` less than `currentLedger`, in such case the transaction submission will return the 
`tefMAX_LEDGER` and transaction won't be accepted by the chain, so the relayers will keep updating the signatures
until they reach the required state.
